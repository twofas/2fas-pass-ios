...
@Suite("WebBrowser V3 → V4 migration", .serialized)
struct WebBrowserEncryptedEntityV3ToV4Tests {
    ...
    private static let v4Model: NSManagedObjectModel = loadModel(named: "ColdStorage4")
    private static let mappingModel: NSMappingModel = {
        guard let mapping = NSMappingModel(
            from: [storageBundle],
            forSourceModel: v3Model,
            destinationModel: v4Model
        ) else {
            fatalError("ColdStorage3→ColdStorage4 custom mapping model not found in Storage framework bundle")
        }
        return mapping
    }()
    ...
    init() {
        MigrationController.current = MigrationController(
            encrypt: { [testAppKey, testMetadataKey] data, key in
                ...
                case .vault:
                    // Vault keys are not exercised by WebBrowser V3→V4; a test that
    ...
    /// Seeds a ColdStorage3 source store with `fixtures`, runs `NSMigrationManager`
    ...
    private func runMigration(tempDir: URL, fixtures: [WebBrowserFixture]) throws -> URL {
        ...
        let manager = NSMigrationManager(sourceModel: Self.v3Model, destinationModel: Self.v4Model)
    ...
    private func readRows(at url: URL) throws -> [NSManagedObject] {
        let coordinator = NSPersistentStoreCoordinator(managedObjectModel: Self.v4Model)
        _ = try coordinator.addPersistentStore(
            ofType: NSSQLiteStoreType,
            configurationName: nil,
            at: url,
